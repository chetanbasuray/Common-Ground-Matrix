...
export const INDICATORS = [
  ...
  { key: 'population', label: 'Population', code: 'SP.POP.TOTL', unit: 'people' },
  { key: 'populationGrowth', label: 'Population Growth', code: 'SP.POP.GROW', unit: '%' },
...
export async function getCountryIndicatorSeries(countryIso, indicatorCode, startYear = 1990) {
  const url = `${BASE_URL}/country/${countryIso}/indicator/${indicatorCode}?format=json&per_page=200&date=${startYear}:2100`;
  const [, rows] = await fetchJson(url);
